...
- 删除Q取色时的像素点学习功能，仅保留Q取色，小像素点现在依靠像素阈值大小来判定，可在调节阈值后使用使用调试模式、按X查看日志、按X截取扫描截图查看绿色像素点来查看小像素点是否被正确识别。取色时你可以在最下方的颜色选择栏直接取色，或者在像素点上取色。
...
- 新增像素阈值快捷键：[ 和 ] ，左方和右方，[为减少，]为增加，快捷键功能可在日志中查看。
...
- 像素阈值基本上不需要修改，默认即可。在十字光标填满填色框的时候，像素阈值15-20也许是最佳效果。
<img width="171" height="125" alt="屏幕截图 2025-10-06 111814" src="https://github.com/user-attachments/assets/24e7768b-693f-4f74-9295-7cfe4a510f77" />

- 地图拉远后空格拖动出现错误拖动，尝试在代码中修改连续的距离阈值，我的设置是35。
- ~~对于边界颜色会变淡一点的像素点，可以Q重新采色，然后W填色。~~
- 被忽略的边界像素点和与地图颜色过于相近，未被识别到的像素点应该不多，手动填一下吧。
